...
public class ContentFilter implements RecomendationFilter {
	...
	public Rating[] doFilter(Dataset<Row> dataSet, int userId, int pageSize) {
		...
		List<Row> result = recommendMovies(dataSet, "Shrek The Third", 20);
		for (Row row : result) {
			System.out.println(
					"###################################################################################################################################");
	...
	public static Dataset<Row> prepareFeatures(Dataset<Row> moviesData) {
		...
		moviesData = moviesData.withColumn("genres_str", expr("array_join(transform(genres, x -> x.name), ' ')"))
				...
				.withColumn("combined_features",
						concat_ws(" ", col("genres_str"), col("keywords_str"), col("cast_str"), col("crew_str")));
	...
	private static Dataset<Row> computeTFIDF(Dataset<Row> moviesData) {
		...
		HashingTF hashingTF = new HashingTF().setInputCol("filtered_words").setOutputCol("raw_features")
				.setNumFeatures(5000);
	...
	public static List<Row> recommendMovies(Dataset<Row> rescaledData, String movieTitle, int numRecommendations) {
		...
		UserDefinedFunction cosineSimilarity = udf((Vector v) -> {
		...
		rescaledData.limit(numRecommendations).withColumn("similarity", cosineSimilarity.apply(col("features")))
		...
		Dataset<Row> similarityData = rescaledData.withColumn("similarity", cosineSimilarity.apply(col("features")))
				.select("title", "similarity").orderBy(col("similarity").desc()) // Order by similarity
				.limit(numRecommendations);

		return similarityData.filter(col("title").notEqual(movieTitle)).orderBy(col("similarity").desc())
				.collectAsList(); // Exclude the input movie itself
	}
}